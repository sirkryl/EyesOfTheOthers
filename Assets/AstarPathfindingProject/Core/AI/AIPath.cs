...
public class AIPath : MonoBehaviour {
	...
	public float TrySearchPath () {
		...
		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && target != null && currDist <= alertDistance) {
			//GetComponent<WalkToDialog>().enabled = true;
			SearchPath ();
			return repathRate;
		} else {
			//StartCoroutine (WaitForRepath ());
			//GetComponent<WalkToDialog>().enabled = false;
...
//		Debug.Log ("target reached");
	...
	public virtual void Update () {
		...
		if (character != null) {
			character.Move(dir, false, false, target.position);
